...
function normalizarItem(item, clientesGuardados) {
  ...
  // El backend usa Precio_Total como total del item (precio × cantidad)
  const cantidad = Number(item.Cantidad_Productos || item.cantidad || 0);
  const precioTotal = Number(item.Precio_Total || item.precioTotal || 0);
  const precioUnitario = cantidad > 0 ? precioTotal / cantidad : Number(producto.Precio_Producto || 0);
