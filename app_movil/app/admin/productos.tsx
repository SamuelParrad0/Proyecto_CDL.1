...
import { listarProductosAdmin, toggleProducto, eliminarProducto, crearProducto, editarProducto } from '@/src/servicios/servicioAdmin';
...
export default function AdminProductosScreen() {
  ...
  const manejarToggle = async (id, estadoActual) => {
    try {
      ...
      setProductos(current => 
        current.map(p => p.Id_Producto === id ? { ...p, Estado_Producto: estadoActual === 1 ? 0 : 1 } : p)
      );
      await toggleProducto(id);
    } catch (error) {
      // Revert on error
      setProductos(current => 
        current.map(p => p.Id_Producto === id ? { ...p, Estado_Producto: estadoActual } : p)
      );
  ...
  const renderProducto = ({ item }) => {
    ...
    const activo = item.Estado_Producto === 1 || item.Estado_Producto === true;
    ...
    return (
      <View style={[styles.tarjeta, !activo && styles.tarjetaInactiva]}>
        ...
        <View style={styles.acciones}>
          <Switch
            value={activo}
            onValueChange={() => manejarToggle(item.Id_Producto, item.Estado_Producto)}
